                    ...
                    lastNotFound.forEach(notFoundTocLink => {
                        ...
                        if (heightRange) {
                            ...
                            if (lineIndex > -1) {
                                ...
                                console.log(' rollupLastNotFound ', headlineType, notFoundTocLink.lineItem )
...
function headlineByLevel2(tocLink) {
    const title = tocLink.lineItem.words[0];
    const computeLevel = title ? title.string.split('.').length : 0;
    if (computeLevel == 1) {
        return BlockType.H2;
    } else if  (computeLevel == 2) {
        return BlockType.H2;
    } else if (computeLevel == 3) {
        return BlockType.H3;
    } else if (computeLevel == 4) {
        return BlockType.H4;
    } else if (computeLevel == 5) {
        return BlockType.H5;
    } else if (computeLevel == 6) {
        return BlockType.H6;
    }

    const level = tocLink.level + 2
    if (level == 1) {
        return BlockType.H1;
    } else if (level == 2) {
        return BlockType.H2;
...
function addHeadlineItems(page, tocLink, foundItems, headlineTypeToHeightRange) {
    ...
    const headlineType = headlineByLevel2(tocLink);


    console.log('addHeadlineItems,', headlineType,page.items, tocLink.lineItem.words);        
